    ...
    public function data_product()
    {
        $products = DB::table('product')
                    ...
                    ->select('product.id','product.sku','product.barcode','product.name','product.category_id','product.creator_id',
                            ...
                            'product.thumbnail','product.price_promo','product.start_promotion','end_promotion','product.created_at',
                ...
                DB::beginTransaction();
                    ...
                    for($j=0;$j<count($detail_sales);$j++){
                        ...
                        DB::table('sale_detail')->updateOrInsert([
                         ...
                         $pd = Product::where('id',$detail_sales[$j]['product_id'])->get();
                         if($pd->count() > 0){
                             $sale_unit = $pd->first()->sale_unit;
                         }else{
                            $sale_unit = 'Pcs';
                         }
